...
passagierfrequenz_df.to_sql('passagierfrequenz', engine, if_exists='replace', index=False)
...
for fahrzeiten_dir in fahrzeiten_dirs:
    ...
    haltestelle_df.to_sql(f'haltestellen_{year}', engine, if_exists='replace', index=False)
    haltepunkt_df = csv_to_df(fahrzeiten_dir + 'haltepunkt.csv')
    haltepunkt_df.to_sql(f'haltepunkte_{year}', engine, if_exists='replace', index=False)
    ...
    #! For test
    fahrzeiten_filenames = random.sample(fahrzeiten_filenames, 10)
    for filename in fahrzeiten_filenames:
        ...
        df.to_sql(f'fahrzeiten_{year}', engine, if_exists='append', index=False, chunksize=chunksize)
